...
import { shortenUrl, checkApiStatus } from "@/services/api"; // Your original API functions
import { testExternalApi } from "@/services/api"; // Add this import
...
const Index = () => {
  ...
  const [externalApiResult, setExternalApiResult] = useState<string | null>(null); // New state for external API

  useEffect(() => {
    const checkApis = async () => {
      // Check your backend API status
      ...
      try {
        ...
        setExternalApiResult(externalResult);
        toast.success("External API test succeeded!");
      } catch (error) {
        setExternalApiResult(
          "External API Error: " + (error instanceof Error ? error.message : "Unknown error")
        );
          ...
          <section className="max-w-xl mx-auto mb-16 animate-fade-in animation-delay-200">
            <div className="glass rounded-2xl p-6 sm:p-8">
              ...
              <AIInput
                ...
                className="mb-2"
                disabled={!apiAvailable} // Disable input if API is unavailable
              />
            ...
            {/* Display external API test result */}
